                ...
                try
                {
                    ...
                    for (int EPOCH = 0; EPOCH < TRAINING_EPOCHS; EPOCH++)    // TRAINING_EPOCHS: quantidade de episódios 
                    {
                        ...
                        for (int STEP = 0; STEP < TRAINING_STEPS; STEP++)    // Duração de cada episodio
                        {
                            ...
                            buffered_reward.Add((_reward + 8) / 8); // Adiciona ao buffer de recompensa action recompensa atual (?) normalizada (reward+8)/8
                            state = step_state;     // Atualiza action variável de estado com o estado recebido pelo ambiente
                            epoch_reward += _reward; // soma action recompensa da ação action recompensa do episodio

                            //  Atualiza PPO //
                            if ((STEP + 1) % NUMBER_OF_SAMPLES == 0 || STEP == TRAINING_STEPS - 1)
                            {
                                double v_s_ = ppo.get_v(step_state);    // Passa o estado atual step_state e recebe o valor atual da taxa de aprendizagem da CRITICA
                                ...
                                {
                                    v_s_ = b_r + GAMMA * v_s_;  // Calcula action recompensa multiplicando action recompensa recebida reward pela GAMMA 
                                                                // e pelo valor da taxa de aprendizado do estado v_s_
                                    discounted_reward.Add(v_s_);     // Adiciona ao array de recompensas calculadas 
                                }
                                ...
                                var bs = np.vstack(buffered_states);
                                var ba = np.vstack(buffered_actions);
                                var _br = np.array(discounted_reward);
