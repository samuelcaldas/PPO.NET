...
class PPO(object):  
    ...
    def __init__(self): # Construtor da Classe
        ...
        with tf.variable_scope('ctrain'):
            ...
            self.ctrain_op = tf.train.AdamOptimizer(CRITIC_LEARNINGRATE).minimize(self.closs)  # Utilizamos o otimizador ADAM, com a taxa de aprendizado da CRITICA C_LR
        ...
        # ATOR:
        #   Politica atual
        pi, pi_params = self.build_actor_network('pi', trainable=True)                  # Criação da rede neural (pi) para a politica atual do ATOR através da função build_anet, definindo como treinável
        ...
        #   Politica antiga
        oldpi, oldpi_params = self.build_actor_network('oldpi', trainable=False)    # Criação da rede neural oldpi para a politica antiga do ATOR através da função build_anet, definindo como não treinável
        ...
        with tf.variable_scope('loss'): # Função de perda:
            with tf.variable_scope('surrogate_pp'):
                ratio = pi.prob(self.action_placeholder) / oldpi.prob(self.action_placeholder)    # O Ratio é a razão da probabilidade da ação tfa na politica nova 
                                                                    # pela probabilidade da ação tfa na politica antiga.
                surr = ratio * self.advantage_placeholder                           # Surrogate é a Razão multiplicada pela vantagem
        ...
        with tf.variable_scope('atrain'):
            self.atrain_op = tf.train.AdamOptimizer(ACTOR_LEARNINGRATE).minimize(self.aloss)  # Utilizamos o otimizador ADAM, com a taxa de aprendizado do ATOR A_LR
